...
const Content = styled.div<{ current?: boolean }>`
  position: relative;
  z-index: 2;
  padding: 10px;
  display: flex;
...
type MapRotatorListType = {
  list: MapsGeneratedList,
  diff_seconds: number,
  filter_maps: number[],
  date: Date,
...
function MapRotatorList({ list, diff_seconds, filter_maps, date, cycle_seconds, advanced }: MapRotatorListType) {
  const [current, setCurrent] = useState<number | null>(null);
  const [currentVisible, setCurrentVisible] = useState(true);

  const preview = (key: number) => {
    if (current === key) {
      setCurrent(null);
      return;
    }
